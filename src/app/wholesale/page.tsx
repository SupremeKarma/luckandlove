...
export default function WholesalePage() {
  ...
  const filtered = React.useMemo(() => {
    ...
    switch (sort) {
      case "fee":   return [...list].sort((a,b)=>(a.delivery_fee ?? Infinity)-(b.delivery_fee ?? Infinity));
      case "time":  return [...list].sort((a,b)=>(a.delivery_time_minutes ?? Infinity)-(b.delivery_time_minutes ?? Infinity));
      case "rating":return [...list].sort((a,b)=>(b.rating ?? -1)-(a.rating ?? -1));
      default:      return list;
    }
  }, [shops, q, sort]);
                ...
                <Card className="rounded-2xl overflow-hidden border-0">
                  ...
                  <CardContent className="space-y-2">
                    ...
                    <div className="text-xs text-muted-foreground flex items-center gap-3">
                      ...
                      <span>•</span>
                      <span>{s.delivery_time_minutes != null ? `${s.delivery_time_minutes} min` : "—"}</span>
                      <span>•</span>
                      <span>{s.delivery_fee != null ? `NPR ${s.delivery_fee.toFixed(2)} fee` : "Free"}</span>
                    </div>
                  </CardContent>
                </Card>
              </Link>
