...
export default function AdminUsersPage() {
  return (
    <div className="container mx-auto px-4 py-12">
      <RequireRole role="admin" redirectTo="/not-authorized" fallback={<p>Loading...</p>}>
        <UsersTable />
      </RequireRole>
    </div>
  );
              ...
              <TableRow key={r.id}>
                <TableCell>
                  <div className="flex items-center gap-3">
                     <Avatar>
                      <AvatarImage src={r.avatar_url ?? undefined} alt={r.full_name ?? 'avatar'} />
                      <AvatarFallback>{r.full_name?.charAt(0) ?? r.email?.charAt(0) ?? 'U'}</AvatarFallback>
                    </Avatar>
                    <div>
                      <div className="font-medium">{r.full_name || "—"}</div>
                      <div className="text-sm text-muted-foreground">{r.email || "—"}</div>
                    </div>
                    {user?.id === r.id && <Badge variant="outline">You</Badge>}
                  </div>
                </TableCell>
                <TableCell>
                ...
                <TableCell className="text-right">
                  <Button
                    size="sm"
                    variant="outline"
                    disabled={busyId === r.id || user?.id === r.id}
                    onClick={() => setRole(r.id, r.role === "admin" ? "user" : "admin")}
                  >
